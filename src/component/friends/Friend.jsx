    ...
    return (
        <Link to={`friends/${id}`} className='p-4 shadow-md '>
            ...
            <div className='flex items-center justify-center mt-2 gap-1.5'>
                {
                    tags.map((tag,index) => <span key={index} className='bg-amber-100'>{tag}</span>)
                }
            </div>
           <button className='flex items-center justify-center mx-auto mt-3 p-1 bg-amber-400 rounded-2xl'>{status}</button>
