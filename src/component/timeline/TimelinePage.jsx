...
const TimelinePage = () => {
    ...
    useEffect(() => {
        if (!filterType || filterType === 'Filter timeline') {
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setFilter(allinteraction);
        }
        else {
            const filterData = allinteraction.filter((info) => info.interactionType.toLowerCase() === filterType.toLowerCase());
            setFilter(filterData);
        }
    }, [filterType, allinteraction])
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <header className="mb-8">
                    ...
                    <div className="relative inline-block text-left">
                        {/* Sorting Interaction */}
                        <select onChange={(e) => SetFilterType(e.target.value)} className="appearance-none bg-white border border-gray-200 text-gray-400 text-sm rounded-lg px-4 py-2.5 pr-10 focus:outline-none focus:ring-2 focus:ring-[#1a4336]/20 cursor-pointer shadow-sm">
                ...
                <div className="flex flex-col gap-10">
                    ...
                    <div>
                        {filter.length === 0 ? <EmptyList /> :
                            <div>
                                {
                                    filter.map((info, index) => <div key={index} className="bg-white p-5 rounded-xl border border-gray-100 shadow-[0_3px_6px_0_rgba(0,0,0,0.05)] flex items-center gap-5 mt-5 hover:translate-x-1 transition-transform cursor-pointer">
